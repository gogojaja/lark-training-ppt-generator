...
### v1.2.0（2026-08-12）
- **新增布局预设 `business_blue_v2`**：商务蓝 v2.1 优化布局
  - 左右两栏布局（主流程左 / 异常分支右），不重叠
  - 精确间距计算，15 节点一页放下
  - 右上角图例 / 肘形分支连线 / 分支编号 E1~E5
  - 现代商务配色：深蓝标题 + 浅蓝步骤 + 橙色判断 + 红色异常
- **布局参数备份**：
  - `_v1_classic_layout_backup.json`：v1 经典布局完整参数备份
  - `gen_flowchart_branch.py.bak_v1`：v1 脚本完整备份
- **经典预设保留**：green / blue / red / yellow 不变
