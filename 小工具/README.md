...
**步骤 0：准备 CSV 节点表（推荐：由 AI 生成）**
- **推荐方式**：调用 flowchart-skill，由 AI 读取 Word 文档内容并语义化输出标准 CSV 节点表，填入下方「CSV 节点表」。
- **辅助方式**：在「〇、CSV 来源」区选择 `.docx` 文档，点击「规则预览」可快速查看离线规则引擎输出（仅作参考，**质量有限**，正式 CSV 不建议直接使用）。
  - 输出 `源文档名_规则预览.csv`（与 Word 同目录）

**步骤 1：选择 CSV 节点表**
1. 点击「浏览…」选择 `.csv` 文件（AI 生成的 CSV 或模板）
